...
export default function App() {
  const [mode, setMode] = useState("login"); // login | admin | user
  ...
  const placeBet = (choice) => {
    ...
    let tempUsers = { ...users };
    tempUsers[currentUser].balance -= 100;
    setUsers(tempUsers);
    setBets([...bets, { user: currentUser, choice }]);
  };

  // Finish round
  const finishRound = () => {
    if (bets.length === 0) return;
    const winning = Math.random() > 0.5 ? "Red" : "Green";
    let tempUsers = { ...users };
    bets.forEach((b) => {
      if (b.choice === winning) {
        tempUsers[b.user].balance += 200;
      }
    });
    setUsers(tempUsers);
    setHistory([{ round: Date.now(), winning, bets }, ...history]);
title: Reference
hideTOC: true
---
